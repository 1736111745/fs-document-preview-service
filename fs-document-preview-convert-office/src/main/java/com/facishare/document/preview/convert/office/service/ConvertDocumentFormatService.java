...
public interface ConvertDocumentFormatService {
   ...
   *
   * @param fileBate 要转换的Excel文件的字节流
   * @param page     要转换的页码
   * @return 返回转换为HTML格式的 单页文档字节流
   */
  byte[] convertOnePageExcelToHtml(byte[] fileBate, int page);

  /**
   * ConvertResultInfo 将word文档或ppt文档 全部转换为PDF格式
   *
   * @param fileBate 要转换的word文档或ppt文档的字节流
   * @param fileType 文件类型枚举 DOC或DOCX
   * @return 返回转换为PDF格式的文档的字节流
   */
  byte[] convertAllPageWordOrPptToPdf(byte[] fileBate, FileTypeEnum fileType);

  /**
   * ConvertResultInfo 转换文档格式 如 ppt转换pptx doc转换为docx xls转换为xlsx
   *
   * @param fileBate 要转换的文档的字节流 （ppt、doc、xlsx)
   * @param fileType 要转换的文档的后缀名
   * @return 成功：返回已经转换好的文档的字节流 失败：返回报错信息（如文件损坏或文件加锁）
   */
  byte[] convertDocumentSuffix(byte[] fileBate, FileTypeEnum fileType);
   ...
   */
  byte[] convertDocumentAllPageToPng(byte[] fileBate, FileTypeEnum fileType);

  /**
   * convertDocumentOnePageToPng 转换一页文档为png
   *
   * @param fileBate 要转换的文档的字节流
   * @param fileType 要转换的文档的后缀名
   * @param page     要转换的文档的页码
   * @return 成功：返回转换为png格式的字节流 失败：返回报错信息（如 目标文件夹路径不存在，源文件不存在）
   */
  byte[] convertDocumentOnePageToPng(byte[] fileBate, FileTypeEnum fileType, int page);
  ...
  byte[] convertDocumentOnePageToPdf(byte[] fileBate, FileTypeEnum fileType, int page);
}
