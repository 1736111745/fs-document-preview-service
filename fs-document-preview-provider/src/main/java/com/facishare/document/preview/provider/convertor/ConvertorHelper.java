...
public class ConvertorHelper {
    ...
    public String toPdf(String filePath) {
        return doConvert(convert -> {
            ...
            byte[] pdfBytes = convert.convertBytesToPDF(bytes);
            if (pdfBytes == null) {
                log.warn("cannot convert ppt to pdf,filePath, {}", filePath);
                return "";
            } else {
                String pdfFilePath = filePath + ".pdf";
                FileUtils.writeByteArrayToFile(new File(pdfFilePath), pdfBytes);
                return pdfFilePath;
            }
        }, filePath);
    }
