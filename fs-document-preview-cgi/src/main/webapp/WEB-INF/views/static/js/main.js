...
var outappid = getQueryStringByName("outappid");
var jumpUrl = getQueryStringByName("jumpUrl");
var pageCount = 0;
var sg = ""//安全组
var width = getQueryStringByName("width");
var pdfConvertType = 0;
var txtAndImageExt = ".sql|.txt|.js|.css|.json|.csv|.svg|.webp|.jpeg|.jpg|.png|.bmp|.gif|.vue|.less|.scss";
...
function getPreviewInfo() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        async: false,
        url: window.contextPath + '/preview/getPreviewInfo?path=' + path + '&token=' + token + "&width=" + width + "&sharetoken=" + sharetoken + "&sg=" + sg + "&outappid=" + outappid,
...
function doPreviewOriginal() {
    $.ajax({
        ...
        dataType: 'json',
        async: false,
        url: window.contextPath + '/preview/getOriginalPreviewInfo?path=' + path + "&sg=" + sg + "&sharetoken=" + sharetoken + "&outappid=" + outappid,
        success: function (data) {
            var dirName = data.dirName;
            var fileName = data.fileName;
            var ea = data.ea;
            var url = window.contextPath + '/preview/' + dirName + "/" + fileName + "?sharetoken=" + sharetoken + "&outappid=" + outappid + "&ea=" + ea;
            location.href = url;
        }
    });
}
...
function doPreviewOffice() {
    ...
    var url = window.contextPath + '/preview/' + route + '?path=' + path + '&pageCount=' + pageCount + "&pdfConvertType=" + pdfConvertType + "&sg=" + sg + "&width=" + width + "&sharetoken=" + sharetoken + "&jumpUrl=" + jumpUrl + "&outappid=" + outappid;
    location.href = url;
}
