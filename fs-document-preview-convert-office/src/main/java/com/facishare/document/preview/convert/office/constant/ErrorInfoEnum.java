...
public enum ErrorInfoEnum {
  FILE_TYPES_DO_NOT_MATCH("400", "文件类型不是受支持的类型"), FILE_PATH_EMPTY("400", "文件路径是空的"), EMPTY_FILE("400", "文件是空的"), FILE_PARAMETER_ERROR("400",
    "没有获取到正确的文件，可能的错误是网络波动导致文件错误的传递"), PAGE_NUMBER_PARAMETER_ZERO("400", "文件损坏了，页码为0"), PAGE_NUMBER_PARAMETER_ERROR("400",
    "页码错误 可能的错误是页码不在文档允许的范围内"), FILE_LINE_EMPTY_ERROR("400", "文件的有效行是空的"), WORD_ENCRYPTION_ERROR("400", "Word 文档对象实例化失败，可能的错误文件加密了"), EXCEL_ENCRYPTION_ERROR(
    "400",
    ...
    "从目标文件路径读取数据失败 可能的错误是目标文件并不存在"), FILE_DELETION_FAILED("500", "删除目标文件夹失败 可能的错误是目标文件夹被其他程序占用"),
