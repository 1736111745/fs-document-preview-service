...
public class OnlineOfficeServerUtil {
    ...
    public byte[] downloadPdfFile(String ea, int employeeId, String path, String sg) {
        ...
        String postUrl = "";
        if (ext.contains("xls") || ext.contains("pdf")) {
            postUrl = String.format(fscServerUrl, ea, String.valueOf(employeeId), path, sg, name);
        } else
            postUrl = ext.contains("ppt") ? generateDownloadUrlForPPT(ea, employeeId, path, sg, name)
    ...
    private String generateDownloadUrlForWordAndPdf(String ea, int employeeId, String path, String sg, String name) {
        ...
        String postUrl = oosServerUrl + "/wv/WordViewer/request.pdf?WOPIsrc=" + URLEncoder.encode(src) + "&access_token=1&access_token_ttl=0&type=printpdf";
        return postUrl;
    }

    private String generateDownloadUrlForPPT(String ea, int employeeId, String path, String sg, String name) {
        String downloadUrl = String.format(fscServerUrl, ea, String.valueOf(employeeId), path, sg, name);
        String src = oosServerUrl + "/oh/wopi/files/@/wFileId?wFileId=" + URLEncoder.encode(downloadUrl) + "&access_token=1&access_token_ttl=0";
