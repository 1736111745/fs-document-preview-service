...
public class TestConvertOfficeToPdf {
  ...
  @ResponseBody
  @RequestMapping(value = "/TestConvertOnePageOfficeToPdf", method = RequestMethod.POST, produces = "application/json")
  public String TestConvertOnePageOfficeToPdf(@RequestParam("path") String path, int page, @RequestParam("file") MultipartFile file) throws IOException {
    byte[] data = toByteArray(file.getInputStream());
    boolean success = convertOffice2Pdf(path, page);
  ...
  @ResponseBody
  @RequestMapping(value = "/TestConvertOfficeToPdf", method = RequestMethod.POST, produces = "application/json")
  public String TestConvertOfficeToPdf(@RequestParam("path") String path, @RequestParam("file") MultipartFile file) throws IOException {
    byte[] data = toByteArray(file.getInputStream());
    boolean success = convertOffice2Pdf(officeConvertorServerUrl, path);
    return JSON.toJSONString(success);
  }
